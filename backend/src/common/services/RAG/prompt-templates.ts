...
Your response:`
   ...
   */
  static generateConversationPrompt(
    context: string,
    query: string,
...
Respond naturally:`
   ...
   */
  private static trimContext(context: string, maxLength: number): string {
  ...
  private static getGeminiInstructions(config: PromptConfig): string {
...
- Avoid repetitive phrases or prefacing every statement`;
  ...
  private static getGPTSystemPrompt(config: PromptConfig): string {
    const responseStyleGuide = {
...
- Avoid repetitive phrases or unnecessary prefacing`;
   ...
   */
  static generateSpecializedPrompt(
    type: 'summary' | 'analysis' | 'qa' | 'creative',
    context: string,
    query: string,
    config: Partial<PromptConfig> = {}
  ): string {
...
export class PromptOptimizer {
   ...
   */
  static calculateOptimalContextLength(query: string, maxTokens: number = 4000): number {
    ...
    if (queryComplexity === 'simple') return Math.floor(baseLength * 0.6);
    if (queryComplexity === 'complex') return Math.floor(baseLength * 0.9);
    return baseLength;
  }
   ...
   */
  private static analyzeQueryComplexity(query: string): 'simple' | 'medium' | 'complex' {
    ...
    if (words > 15 || hasMultipleQuestions || hasComplexTerms) return 'complex';
    return 'medium';
  }
